...
it("should see only null task, if unauth", () => {
  const url = Cypress.env("BACKEND_URL");
...
it("shouldn't modify my todo, if unauth", () => {
  const url = Cypress.env("BACKEND_URL");
...
it("user B cannot modify user A's owned todo", () => {
  const url = Cypress.env("BACKEND_URL");
...
it("should see only own task even fliter", () => {
  const url = Cypress.env("BACKEND_URL");
  const username = `user_${Date.now()}`;
  const password = "p@ssw0rd";
